...
def connection():
    ...
    return conn
#GENERA EXCEL  DE LOS  ESTUDIANTES REGISTRADOS  EN EL SISTEMA
def exportExcel():
    conn=connection()
    cursor=conn.cursor()
...
# MOSTRARA TODO LOS DATOS REGISTRADOS EN LA BASE  DE  DATOS  ALA  VISTA DE PYTHON
def read():
    conn=connection()
    cursor=conn.cursor()
    cursor.connection.ping()
...
def deleteStudent():
    try:
        ...
        messagebox.showwarning("Info", "SELECCIONE  UNA  FILA  POR FAVOR")
        return
    decision = messagebox.askquestion("Warning", "DESEA ELIMINAR EL ESTUDIANTE SELECCIONADO?")
    if decision != "yes":
        return 
    else:
        try:
...
def renderAddWindow():
    ...
    def addStudent():
        ...
        qrCodeFilename = f"{generate_random_string()}.png"
        ...
        if (studid == "" or studid == " ") or (fname == "" or fname == " ") or (lname == "" or lname == " ") or (phone == "" or phone == " ") or (email == "" or email == " ") or (address == "" or address == " "):
            ...
            try:
                ...
                qr_img = qr_img.resize((300, 300), resample=Image.LANCZOS) 
                qr_img.save(f"./assets/uploaded/{qrCodeFilename}")

                if os.path.exists("./assets/uploaded/temp.png"):
                    os.remove("./assets/uploaded/temp.png")
                conn = connection()
                cursor = conn.cursor()
                cursor.execute(f"INSERT INTO students (profile_image, stud_id, fname, lname, phone, email, address, date_update, codigo_qr) VALUES ('{imageProfileFilename}', '{studid}', '{fname}', '{lname}', '{phone}', '{email}', '{address}', '{getCurrentDate()}', '{qrCodeFilename}')")
                conn.commit()
                conn.close()
            except:
                messagebox.showinfo("Error", "ESTUDIANTE CON EL ID YA EXISTE", parent=addCanvas)
                return
    ...
    addWindow = Tk()
    addWindow.title('AGREGAR NUEVO ESTUDIANTE-IESTP ACORA')
    ...
    addCanvas.create_text(49.0, 10.0, anchor="nw", text="NUEVO ESTUDIANTE", fill="#FFFFFF", font=("Inter SemiBold", 24 * -1))
...
def renderEditWindow(selectedStudData):
    ...
    conn.close()
    editWindow = Tk()
    editWindow.title('EDITAR ESTUDIANTE-IESTP ACORA')
    ...
    editCanvas.create_text(49.0, 10.0, anchor="nw", text="EDITAR ESTUDIANTE", fill="#FFFFFF", font=("Inter SemiBold", 24 * -1))
...
def renderViewWindow(selectedStudData):
    ...
    viewWindow = Tk()
    viewWindow.title('VER ESTUDIANTE-IESTP ACORA')
    ...
    viewCanvas.create_text(49.0, 10.0, anchor="nw", text="VISTA DE ESTUDIANTE", fill="#FFFFFF", font=("Inter SemiBold", 24 * -1))
    ...
    studidEntry.place(x=350.0, y=180.0, width=260.0, height=21.0)
    ...
    fnameEntry.place(x=350.0, y=200.0, width=260.0, height=21.0)
    ...
    lnameEntry.place(x=350.0,y=220.0,width=260.0,height=21.0)
    ...
    phoneEntry.place(x=350.0,y=240.0,width=260.0,height=21.0)
    ...
    emailEntry.place(x=350.0,y=260.0,width=260.0,height=21.0)
    ...
    addressEntry.place(x=350.0, y=380.0, width=551.0, height=21.0)
    addressEntry.bind("<Key>", lambda e: "break")
    viewCanvas.create_text(350.0,339.0,anchor="nw",text="Address",fill="#000000",font=("Inter", 11 * -1))
    addressEntry.insert(0,address)
    

    image_image_10 = PhotoImage(master=viewWindow, file=viewWindowAssets("image_12.png"))
    viewCanvas.create_image(355.0, 263.0, image=image_image_10)
    
    image_image_11 = PhotoImage(master=viewWindow, file=f"./assets/uploaded/{result[9]}")
    viewCanvas.create_image(200, 280, image=image_image_11)
...
def scanearQr():
    ...
    while True:
        ...
        for barcode in decode(frame):
            ...
            try:
                ...
                cursor.execute("INSERT INTO asistencias (qr_data, fecha_hora) VALUES (%s, %s)", (qr_data, fecha_hora_actual))
                connection.commit()
                messagebox.showinfo("Código QR Escaneado", f"REGISTRADO: {qr_data}\nFecha y Hora: {fecha_hora_actual}")
            except mysql.connector.Error as e:
                messagebox.showerror("Error", f"Error al registrar la fecha y hora: {e}")
            finally:
    ...
    cv2.destroyAllWindows()
#############################################################################################
#  INTERFACE  GRAFICA   DEL APLICATIVO
mainWindow = Tk()
mainWindow.title('INSTITUTO  DE EDUCACION SUPERIOR TECNOLOGICO PUBLICO DE ACORA')
...
mainCanvas.create_text(73.0,15.0,anchor="nw",text="REGISTRO DE ESTUDIANTE ",fill="#FFFFFF",font=("Inter SemiBold", 36 * -1))
...
button_image_6 = PhotoImage(file=mainWindowAssets("button_6.png"))
button_6 = Button(image=button_image_6,borderwidth=0,highlightthickness=0,command=scanearQr,relief="flat")
button_6.place(x=28.0,y=515.0,width=148.0,height=57.0)
